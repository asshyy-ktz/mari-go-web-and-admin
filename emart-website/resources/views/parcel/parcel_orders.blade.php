    ...
    function buildHTMLPendingOrders(completedorderSnapshots) {
        ...
        alldata.forEach((listval) => {
            ...
            if (val.status == "Order Placed" || val.status == "Order Accepted" || val.status == "Driver Pending" || val.status == "Order Shipped" || val.status == "In Transit") {
                ...
                var discount = '';
                if (currencyAtRight) {
                    order_total_val = order_total.toFixed(decimal_degits).toLocaleString('hi-IN') + '' + currentCurrency;
                    subtotal = order_subtotal.toFixed(decimal_degits).toLocaleString('hi-IN') + '' + currentCurrency;
                    ...
                    order_total_val = currentCurrency + '' + order_total.toFixed(decimal_degits).toLocaleString('hi-IN');
                    subtotal = currentCurrency + '' + order_subtotal.toFixed(decimal_degits).toLocaleString('hi-IN');
    ...
    function buildHTMLRejectedOrders(completedorderSnapshots) {
        ...
        alldata.forEach((listval) => {
            ...
            if (val.status == "Driver Rejected" || val.status == "Order Rejected") {
                ...
                if (currencyAtRight) {
                    weightCharge = val.parcelWeightCharge.toFixed(decimal_degits).toLocaleString('hi-IN') + '' + currentCurrency;
                } else {
                    weightCharge = currentCurrency + '' + parseFloat(val.parcelWeightCharge).toFixed(decimal_degits).toLocaleString('hi-IN');
                }
